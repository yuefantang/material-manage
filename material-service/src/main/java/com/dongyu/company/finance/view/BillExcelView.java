...
@Slf4j
@Component
public class BillExcelView extends ExcelView {
    ...
    protected void setRow(Workbook workbook, Map<String, Object> map) {
        ...
        //需要导出的数据源
        List<BillListDTO> listDTO = (List<BillListDTO>) map.get("billExportList");
            ...
            for (BillListDTO data : listDTO) {
                ...
                if (rowCount == 0 || rowCount >= Constants.MAX_XLS_SHEET_ROW) {
                    ...
                    headers.addAll(Arrays.asList("DY编号", "客户名称", "客户型号", "客户订单号",
                            "送货单号", "送货数量", "单价(单位元)", "金额（单位元）", "投产单号", "送货日期", "对账月份","核实状态"));
                ...
                cell12.setCellValue(Optional.ofNullable(data.getBillMonth()).orElse(""));

                 //核实状态
                Cell cell13 = dataRow.createCell(creditNum++);
                cell13.setCellStyle(cellStyle);
                cell13.setCellValue(Optional.ofNullable(data.getVerifyState()).orElse(""));
